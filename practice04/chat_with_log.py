...
def extract_5w_info(config: dict, messages: list[dict]) -> list[str]:
    ...
    prompt = [
        {
            "role": "system",
            "content": (
                "你是一个信息提取助手。请从以下对话中提取关键信息，按照5W格式输出。\n"
                "每条信息格式为: [Who:xxx] [What:xxx] [When:xxx] [Where:xxx] [Why:xxx]\n"
                "When和Where如无法确定可填'N/A'，Why如不明确可填'N/A'。\n"
                "请提取所有有价值的关键信息，每条一行。至少提取1条。直接输出，不要解释。"
            )
        }
    ]
    prompt.extend(conversation)
    prompt.append({"role": "user", "content": "请从以上对话中提取关键信息，按5W格式输出。"})
    ...
    try:
        result = _request(base_url, api_key, model, prompt, tools=None,
                         temperature=temperature, max_tokens=min(max_tokens, 512), timeout=timeout)
...
def append_to_log(entries: list[str]) -> str:
    if not entries:
        return "无关键信息需要记录"
    _ensure_log_file()
    ...
    with open(LOG_FILE, "a", encoding="utf-8") as f:
        ...
        for entry in entries:
            f.write(entry + "\n")
    return f"已记录 {len(entries)} 条关键信息到 {LOG_FILE}"


def extract_and_log(config: dict, messages: list[dict]) -> None:
    print(f"\n  🔍 触发5W关键信息提取 (每{EXTRACT_INTERVAL}轮)...")
    entries = extract_5w_info(config, messages)
    if entries:
        print(f"  📋 提取到 {len(entries)} 条关键信息:")
        for e in entries:
            print(f"     {e}")
        result = append_to_log(entries)
        print(f"  💾 {result}")
    else:
        print(f"  ⚠️ 本轮未提取到关键信息")


def search_chat_log(query: str = "") -> str:
    if not os.path.exists(LOG_FILE):
        return "[提示] 聊天日志文件尚不存在，暂无历史记录可供搜索。请先进行一些对话。"
    try:
        with open(LOG_FILE, "r", encoding="utf-8") as f:
            content = f.read()
        if not query:
            return f"[聊天日志] {LOG_FILE}:\n\n{content[-3000:]}"
        return f"[聊天日志] {LOG_FILE}:\n\n{content}\n\n---日志结束---\n\n请根据以上聊天历史日志，回答用户的查询。如果日志中无相关信息，请如实说明。"
    except Exception as e:
...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "搜索聊天历史日志。当用户用'/search'开头、表达了查找聊天历史的意思，或AI认为需要回顾历史对话时使用。参数query为搜索关键词或问题。",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {"type": "string", "description": "搜索查询内容，即用户想从历史日志中查找的关键词或问题"}
                },
                "required": ["query"]
            }
        }
...
def main():
    ...
    system_prompt = (
        ...
        "- 如果用户用'/search'开头、查找聊天历史、或需要回顾之前对话 → 调用 search_chat_log\n"
        "- 如果问题不需要工具，可以直接回答"
    )
    ...
    print("=" * 55)
    print("  📚 Practice 04 - 聊天日志与5W关键信息提取")
    print(f"  模型: {model}")
    print(f"  提取频率: 每{EXTRACT_INTERVAL}轮提取一次5W关键信息")
    print(f"  日志文件: {LOG_FILE}")
    print(f"  搜索功能: /search + 关键词")
    print("=" * 55)
    ...
    try:
        while True:
            ...
            if user_input.startswith("/search"):
                ...
                if not search_query:
                    print("[系统] 请输入搜索关键词，如: /search 昨天讨论了什么")
                    continue
                log_content = search_chat_log(search_query)
                if log_content.startswith("[提示]"):
                    print(f"[系统] {log_content}")
                    continue
                search_messages = [{"role": "system", "content": system_prompt}]
                search_messages.append({
                    "role": "user",
                    "content": f"用户想要查找聊天历史中的信息。搜索查询: {search_query}\n\n{log_content}"
                })
                print(f"\n  🔍 搜索聊天历史: {search_query}")
                try:
                    result = _request(base_url, api_key, model, search_messages, TOOLS, temperature, max_tokens, timeout)
                    print(f"\n[AI] {result.get('content', '(空)')}")
                    print(f"  📊 tokens: {result['prompt_tokens']}+{result['completion_tokens']} | ⏱ {result['elapsed_seconds']:.2f}s")
                except Exception as e:
                    print(f"[错误] 搜索失败: {e}")
                continue
            ...
            if round_count % EXTRACT_INTERVAL == 0:
                extract_and_log(config, messages)
