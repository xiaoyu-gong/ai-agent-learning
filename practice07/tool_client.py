...
def load_env():
        ...
        for line in f:
            ...
            if line and not line.startswith('#'):
                if '=' in line:
                    ...
                    raw_config[key.strip()] = value.strip().strip('"').strip("'")
    ...
    for src_key, dst_key in env_map.items():
        if src_key in raw_config and not os.environ.get(dst_key):
            os.environ[dst_key] = raw_config[src_key]
...
class ChainedCallContext:
    ...
    def to_text(self):
        lines = []
        for s in self.steps:
            lines.append(f"步骤{s['step']}: 调用 {s['tool_name']}({json.dumps(s['arguments'], ensure_ascii=False)})")
            result_preview = s['result'][:300] + '...' if len(s['result']) > 300 else s['result']
            lines.append(f"  结果: {result_preview}")
        return '\n'.join(lines) if lines else '(暂无步骤)'


def build_analysis_prompt(user_request, context):
    history_text = context.to_text()
    return f"""## 用户请求
{user_request}
...
## 决策规则
1. 仔细分析用户请求和已执行的步骤结果
2. 判断任务是否已完成（所有必要的工具调用已执行完毕）
3. 如果已完成，提供最终回答
4. 如果未完成，决定下一步需要调用哪个工具

## JSON输出格式
- 任务完成时：{{"done": true, "answer": "最终回答"}}
- 继续调用工具时：{{"done": false, "tool_call": {{"name": "工具名", "arguments": {{"参数": "值"}}}}}}

请根据以上信息，输出你的决策JSON。"""


def execute_chained_tool_call(user_request, system_prompt, tools):
    context = ChainedCallContext(max_iterations=10)
    messages = [{"role": "system", "content": system_prompt}]
    messages.append({"role": "user", "content": user_request})

    print(f"\n{'='*50}")
    print(f"  链式调用开始: {user_request[:60]}...")
    print(f"{'='*50}")
    ...
    while not context.is_max_reached():
        ...
        decision_messages.append({"role": "user", "content": analysis_prompt})

        print(f"\n  [{context.iteration+1}/{context.max_iterations}] 请求LLM决策...")
        response = call_llm(decision_messages, tools)
        ...
        if tool_calls:
            ...
            try:
                ...
                if '"done"' in clean:
                    done_m = re.search(r'"done"\s*:\s*(true|false)', clean)
                    answer_m = re.search(r'"answer"\s*:\s*"([^"]*)"', clean)
                    if done_m and answer_m:
                        decision = {"done": done_m.group(1) == 'true', "answer": answer_m.group(1)}
                if not decision:
                    return f"错误：无法解析LLM响应。原始内容:\n{content[:500]}"

        if not decision:
            return "错误：LLM未返回有效决策"

        if decision.get("done"):
            return decision.get("answer", content)

        tc_info = decision.get("tool_call", {})
        tool_name = tc_info.get("name", "")
        tool_args = tc_info.get("arguments", {})

        if not tool_name:
            return "错误：决策中缺少工具名称"

        print(f"  🔧 调用工具: {tool_name}({json.dumps(tool_args, ensure_ascii=False)})")
        tool_result = _execute_by_name(tool_name, tool_args)
        context.add_step(tool_name, tool_args, tool_result)
        print(f"  📋 结果摘要: {tool_result[:200]}...")

        messages.append({"role": "assistant", "content": content or ''})
        messages.append({"role": "tool", "tool_call_id": tool_call_id, "content": tool_result})

    return f"已达到最大迭代次数({context.max_iterations})，最后状态:\n{context.to_text()}"


def _execute_by_name(tool_name, tool_args):
    import json as _json
    if tool_name == "list_directory":
        return list_files(tool_args.get('path', '.'))
    elif tool_name == "rename_file":
        return rename_file(tool_args.get('directory'), tool_args.get('old_name'), tool_args.get('new_name'))
    elif tool_name == "delete_file":
        return delete_file(tool_args.get('directory'), tool_args.get('file_name'))
    elif tool_name == "create_file":
        return create_file(tool_args.get('directory'), tool_args.get('file_name'), tool_args.get('content'))
    elif tool_name == "read_file":
        return read_file(tool_args.get('directory'), tool_args.get('file_name'))
    elif tool_name == "fetch_webpage":
        return fetch_webpage(tool_args.get('url'))
    elif tool_name == "search_chat_history":
        return search_chat_history(tool_args.get('query'))
    elif tool_name == "anythingllm_query":
        return anythingllm_query(tool_args.get('message'))
    elif tool_name == "list_available_skills":
        return list_available_skills()
    elif tool_name == "load_skill_content":
        return load_skill_content(tool_args.get('skill_name'))
    else:
...
def list_available_skills() -> str:
    if not os.path.isdir(SKILLS_DIR):
        return json.dumps({"status": "error", "message": f"技能目录不存在: {SKILLS_DIR}"}, ensure_ascii=False)
    ...
    for name in sorted(os.listdir(SKILLS_DIR)):
        ...
        fm_name = re.search(r'name:\s*(.+)', front_matter)
        fm_desc = re.search(r'description:\s*(.+)', front_matter)
        skills.append({
            "name": fm_name.group(1).strip() if fm_name else name,
            "description": fm_desc.group(1).strip() if fm_desc else ""
        })
...
def load_skill_content(skill_name: str) -> str:
    if not skill_name:
        return json.dumps({"status": "error", "message": "未指定技能名称"}, ensure_ascii=False)

    skill_path = os.path.join(SKILLS_DIR, skill_name, "SKILL.md")
    if not os.path.isfile(skill_path):
        return json.dumps({"status": "error", "message": f"技能 {skill_name} 不存在: {skill_path}"}, ensure_ascii=False)
...
def main():
    ...
    tools = [
        ...
        },
        {
            ...
            "function": {
                ...
                "description": "读取项目目录下所有可用的技能(skill)列表。每次用户提交输入时自动调用，返回所有已安装技能的名称和描述。",
                "parameters": {
                    "type": "object",
                    "properties": {},
                    "required": []
        ...
        },
        {
            ...
            "function": {
                ...
                "description": "加载指定技能的完整内容(正文)。当LLM判断需要使用某个技能时调用此函数获取该技能的SKILL.md正文，然后遵照执行。",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "skill_name": {
                            "type": "string",
                            "description": "技能名称，如 notice"
                        }
                    },
                    "required": ["skill_name"]
                }
    ...
    skills_list_raw = list_available_skills()
    skills_data = json.loads(skills_list_raw)
    skills_json = json.dumps(skills_data.get("skills", []), ensure_ascii=False)

    system_prompt = f"""你是一个具有文件操作、网络访问、聊天历史搜索、文档仓库访问和技能调用能力的AI助手。
...
1. 每次收到用户请求后，首先使用 list_available_skills 工具获取当前可用的技能列表。
2. 根据用户的需求，判断是否需要使用某个技能。如果需要，调用 load_skill_content 工具加载该技能的完整内容。
3. 加载技能内容后，严格按照技能中定义的规则执行用户请求。
4. 如果用户没有提到任何与技能匹配的需求，可以作为普通对话处理。
...
当用户要求撰写通知、修改通知、润色通知时，应使用 notice 技能。
如果用户没有告知所在的部门，使用"XX部"代替。

## 链式工具调用规则

当用户的请求需要多个连续的步骤（前一步的结果是后一步的输入）时，使用链式调用模式：
1. 先调用第一个工具获取中间结果
2. 根据中间结果，决定下一步调用哪个工具
3. 将中间结果作为上下文变量，传递给后续的工具调用
4. 每一步完成后都要评估任务是否已完成
5. 如果任务完成，提供最终答案；否则继续下一步

链式调用示例：
- 搜索文件 → 读取文件 → 分析内容 → 创建总结文件
- 读取两个文件 → 计算结果 → 写入结果文件
- 访问网页 → 提取内容 → 保存到文件
...
当使用 anythingllm_query 工具访问文档仓库后，你可以根据返回的结果向用户提供相关信息。"""
    ...
    print("=== LLM 工具调用客户端 (Practice 07 - 链式工具调用) ===")
    print("输入消息开始聊天，按 Ctrl+C 退出")
    print("=======================")
    print("可用工具：list_directory, rename_file, delete_file, create_file, read_file, fetch_webpage, search_chat_history, anythingllm_query, list_available_skills, load_skill_content")
    print()
    print("模式说明：")
    print("  /chain <请求>    — 使用链式调用模式（多步骤自动执行）")
    print("  直接输入请求     — 正常对话模式")
    print()
    print("链式调用示例：")
    print("  /chain 读取1.txt和2.txt，把两个数相加写入result.txt")
    print("  /chain 访问网页并总结内容保存到文件")
    print()
        ...
        while True:
            ...
            if user_input.startswith('/chain'):
                ...
                if not chain_request:
                    print("请提供链式调用请求，如: /chain 读取1.txt和2.txt，把两个数相加写入result.txt")
                    continue
                print(f"助手: ", end='', flush=True)
                result = execute_chained_tool_call(chain_request, system_prompt, tools)
                if result:
                    print(result)
                print()
                continue
